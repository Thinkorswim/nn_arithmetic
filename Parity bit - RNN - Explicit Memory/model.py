...
def import_data(dataset="data"):
    ...
    for line in f.readlines():
            ...
            halfLength = int(len(intLine)/2)
            X.append(intLine[:halfLength])
            Y.append(intLine[halfLength:])
    ...
    Y = np.array(Y)
    Y = X.reshape(Y.shape[0],1 ,Y.shape[1])

    return X,Y

def create_RNN_model(input_shape):
    model = Sequential()
    model.add(LSTM(input_shape[2]+1, input_shape=(input_shape[1], input_shape[2]), activation="relu", return_sequences=True))
...
if __name__ == '__main__':
    ...
    # classifier.fit(X_train, Y_train, validation_data=(X_test, Y_test), epochs=epochs, batch_size=b_size, verbose=1)
    # result = classifier.evaluate(X_test, Y_test, batch_size=b_size)
